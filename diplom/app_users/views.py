...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.POST['profile_status'] == 'cu':
...
class CustomerDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['tasks'] = TaskModel.objects.filter(user=self.object)
        return context
...
class ContractorDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['tasks'] = ContractorTask.objects.filter(user=self.object, point__gt=0)
        return context
